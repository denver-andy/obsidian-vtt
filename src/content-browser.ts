...
// Each item: padding(6+6) + thumb(48) = 60px box + margin(2+2) = 64px total in flex column
...
export class ContentBrowser {
	...
	private async showPanel(id: ContentCategory) {
		if (!this.panelCache.has(id)) {
			...
			if (!this.panelCache.has(id)) { // re-check: concurrent call may have finished first
	...
	private createPanel(category: ContentCategory, allFiles: AssetFile[]) {
		...
		// containerEl has a fixed height = total * ITEM_HEIGHT and is never resized during scroll,
		// which prevents the feedback loop that occurs when scroll height changes mid-scroll.
	...
	private applyFilters(state: PanelState) {
		...
		// Set container height once — never touched during scroll — so the scrollbar
	...
	private updateVirtualWindow(state: PanelState) {
		...
		// Absolutely position the items window — container height stays fixed,
	...
	private getCachedFiles(category: ContentCategory): Promise<FolderNode> {
		...
		if (!this.fileCache.has(key)) {
			// Store the Promise itself so concurrent calls share the same walk.
